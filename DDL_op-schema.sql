...
alter table products 
	add constraint products_fk1 foreign key (supplier_id) references suppliers (supplier_id),
...
alter table orders 
	...
	add constraint orders_fk2 foreign key (employee_id) references employees (employee_id),
	add constraint orders_fk3 foreign key (shipper_id) references shippers (shipper_id);
...
--auto increments for every primary key in database


-- auto increments
alter table categories alter category_id add generated always as identity;
SELECT SETVAL(pg_get_serial_sequence('categories', 'category_id'), max(category_id)) from categories;

-- check for auto increments (using sequences)
insert into categories(category_name, description) values
('Energy Drink', 'Caffein and tuffs');

alter table customers alter customer_id add generated always as identity;
SELECT SETVAL(pg_get_serial_sequence('customers', 'customer_id'), max(customer_id)) from customers;

alter table employees alter employee_id add generated always as identity;
SELECT SETVAL(pg_get_serial_sequence('employees', 'employee_id'), max(employee_id)) from employees;

alter table order_details alter order_detail_id add generated always as identity;
SELECT SETVAL(pg_get_serial_sequence('order_details', 'order_detail_id'), max(order_detail_id)) from order_details;

alter table orders alter order_id add generated always as identity;
SELECT SETVAL(pg_get_serial_sequence('orders', 'order_id'), max(order_id)) from orders;

alter table products alter product_id add generated always as identity;
SELECT SETVAL(pg_get_serial_sequence('products', 'product_id'), max(product_id)) from products;

alter table shippers alter shipper_id add generated always as identity;
SELECT SETVAL(pg_get_serial_sequence('shippers', 'shipper_id'), max(shipper_id)) from shippers;

alter table suppliers alter supplier_id add generated always as identity;
SELECT SETVAL(pg_get_serial_sequence('suppliers', 'supplier_id'), max(supplier_id)) from suppliers;
